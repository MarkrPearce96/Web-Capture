...
- **Tools**: Select, Pen (freehand), Highlighter, Line, Arrow, Rectangle, Ellipse, Text. Selecting a tool switches the image area from its normal scroll/pan behavior to drawing mode (crosshair cursor); clicking the active tool again deselects it and returns to normal scrolling. No tool is selected by default.
